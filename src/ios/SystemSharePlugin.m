...
- (NSString*)getIPadPopupCoordinates {
  // see https://github.com/EddyVerbruggen/SystemSharePlugin-PhoneGap-Plugin/issues/1052
  return nil;
}
...
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
    ...
    NSArray * SystemSharePluginExcludeActivities = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"SystemSharePluginExcludeActivities"];
    if (SystemSharePluginExcludeActivities!=nil && [SystemSharePluginExcludeActivities count] > 0) {
      activityVC.excludedActivityTypes = SystemSharePluginExcludeActivities;
    }
...
- (void)shareViaEmail:(CDVInvokedUrlCommand*)command {
  if ([self isEmailAvailable]) {
    ...
    if (TARGET_IPHONE_SIMULATOR && IsAtLeastiOSVersion(@"8.0")) {
      UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"SystemSharePlugin plugin"
      ...
      for (NSString* path in attachments) {
        ...
        NSString* basename = [self getBasenameFromAttachmentPath:path];
          ...
          else
          {
              ...
              data = [SystemSharePlugin dataFromBase64String:base64content];
          }
  ...
  if (fileName != (id)[NSNull null]) {
    ...
    if ([fileName hasPrefix:@"http"]) {
      ...
      file = [NSURL fileURLWithPath:[fileName substringFromIndex:6]];
        ...
        NSData* data = [SystemSharePlugin dataFromBase64String:base64content];
...
- (void) documentInteractionController: (UIDocumentInteractionController *) controller willBeginSendingToApplication: (NSString *) application {
  // note that the application actually contains the app bundle id which was picked (for whatsapp and instagram only)
      NSLog(@"SystemSharePlugin app selected: %@", application);
}
